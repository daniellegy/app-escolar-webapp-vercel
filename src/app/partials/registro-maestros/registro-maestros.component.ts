...
export class RegistroMaestrosComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.activatedRoute.snapshot.params['id'] != undefined) {
      ...
      // Convertir materias_json de string a array si es necesario
      if (
        this.maestro.materias_json &&
        typeof this.maestro.materias_json === 'string'
      ) {
        try {
          this.maestro.materias_json = JSON.parse(this.maestro.materias_json);
        } catch (e) {
          console.error('Error parsing materias_json:', e);
          this.maestro.materias_json = [];
        }
      } else if (!this.maestro.materias_json) {
        this.maestro.materias_json = [];
      }
    } else {
      this.editar = false;
